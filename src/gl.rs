...
impl Window {
    ...
    const EXIT_ON_ESC: u32 = 1;
    const RESIZABLE: u32 = 2;
    const VSYNC: u32 = 4;
    const DECORATED: u32 = 8;
    const FULLSCREEN: u32 = 16;
    const SRGB: u32 = 32;
    const CONTROLLERS: u32 = 64;
    const LAZY: u32 = 128;
    ...
    pub fn begin(&mut self, title: String, FLAGS: u32) {
        ...
        self.window = Some(
            WindowSettings::new("", [self.width as u32, self.height as u32])
                .opengl(OPENGL_VER)
        ...
        self.events = self.events.lazy(FLAGS | Self::LAZY != 0);
    ...
    pub fn render(&mut self, args: &RenderArgs) {
        ...
        if let Some(f) = &mut self.renderf {
            self.gl
                ...
                .draw(args.viewport(), |c, gl| f(self.rtick, self.utick, c, gl));
        }
        self.rtick += 1;
    }
    ...
    pub fn set_render_fn(
        &mut self,
        new_renderf: Box<dyn FnMut(u64, u64, graphics::Context, &mut GlGraphics)>,
    ) {
...
mod tests {
    ...
    fn it_works() {
        ...
        const DEFAULT_FLAGS: u32 =
            Window::EXIT_ON_ESC | Window::RESIZABLE | Window::VSYNC | Window::DECORATED;
        dbg!(DEFAULT_FLAGS & Window::FULLSCREEN != 0);
        dbg!(DEFAULT_FLAGS & Window::DECORATED != 0);
        dbg!(DEFAULT_FLAGS & Window::VSYNC != 0);
